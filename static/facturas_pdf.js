...
function previsualizarFactura() {
    ...
    const renderizarAnexo = (archivos, etiqueta, contenedor) => { Array.from(archivos).forEach(file => { const divInfo = document.createElement('div'); divInfo.className = 'pdf-image-container'; if (file.type.startsWith('video/')) { divInfo.innerHTML = `<div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:120px; background:#f3f4f6; border:1px solid #d1d5db; width:100%; border-radius:5px;"><div style="font-size:40px; margin-bottom:5px;">🎥</div><p class="pdf-anexo-label" style="font-weight:bold;">${etiqueta} (Video): ${file.name}</p></div>`; } else if (file.type.startsWith('image/')) { divInfo.innerHTML = `<img src="${URL.createObjectURL(file)}" class="pdf-anexo-img"><p class="pdf-anexo-label">${etiqueta}: ${file.name}</p>`; } else if (file.name.toLowerCase().endsWith('.pdf')) { divInfo.innerHTML = `<div style="font-size:40px; color:#ef4444; margin-bottom:10px;">📄</div><p class="pdf-anexo-label">${etiqueta} (PDF Adjunto): ${file.name}</p>`; } contenedor.appendChild(divInfo); }); };
...
function generarPDFSilencioso(idFactura) {
    ...
    f.fotos_evidencia.forEach(foto => { let ruta = `/static/facturas_archivos/${foto}`; let fLower = foto.toLowerCase(); if (fLower.endsWith('.mp4') || fLower.endsWith('.mov') || fLower.endsWith('.webm') || fLower.endsWith('.avi')) { contEvidencia.innerHTML += `<div class="pdf-image-container" style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:120px; background:#f3f4f6; border:1px solid #d1d5db; width:100%; border-radius:5px;"><div style="font-size:40px; margin-bottom:5px;">🎥</div><p class="pdf-anexo-label" style="font-weight:bold;">Video Adjunto: ${foto}</p></div>`; } else { contEvidencia.innerHTML += `<div class="pdf-image-container"><img src="${ruta}" class="pdf-anexo-img"><p class="pdf-anexo-label">Evidencia: ${foto}</p></div>`; } });
